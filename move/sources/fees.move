/// Module: fees
///
/// Pure split computation + balance distribution. Config-agnostic — callers
/// pull the bps values from LaunchpadConfig themselves and pass them in.
/// This decouples `tai::fees` from `tai::launchpad` so the latter can freely
/// import the former without creating a dependency cycle.
///
/// `Split` carries (nav, creator, platform_or_burn). For trade and
/// service-SUI flows, `platform_or_burn` is the platform share. For
/// service-token flows, it is the burn share. In all cases it also absorbs
/// the rounding remainder so the three parts sum to `total` exactly.
module tai::fees {
    ...
    /// Split `total` into (nav, creator, remainder) per the provided bps.
    ...
    ///   - the remainder (burn share + any rounding) is BURNED via `cap`.
